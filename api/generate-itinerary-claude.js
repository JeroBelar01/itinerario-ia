// ARCHIVO TEMPORAL DE PRUEBA — copia de generate-itinerary.js (mismo
// systemPrompt/userPrompt/criterio real, para comparar en igualdad de
// condiciones) pero llamando a Claude (Anthropic) en vez de Gemini. No
// sustituye a generate-itinerary.js: ese sigue intacto y es el que usan todos
// los usuarios normales. Este solo se usa cuando la app se abre con
// "?motor=claude" en la URL (ver index.html). Se puede borrar sin problema
// en cuanto terminemos de comparar.
...
Debes llamar SIEMPRE a la herramienta "generar_itinerario" con el itinerario completo. El array
"dias" debe tener exactamente un objeto por cada día del itinerario, ni uno más ni uno menos.`;
...
Recuerda: llama a la herramienta con el itinerario pedido, y sé conciso en cada campo para no cortar la respuesta.`;

  const maxTokens = Math.min(8000, 1400 + days * 750);
  ...
  try {
    ...
    // LOG TEMPORAL: para poder comparar tiempos reales en los logs de Vercel
    // igual que hacemos con Gemini.
    console.log('[generate-itinerary-claude][TIMING]', JSON.stringify({
      durationSegundos, days, httpStatus: response.status,
      inputTokens: data.usage?.input_tokens ?? null,
      outputTokens: data.usage?.output_tokens ?? null,
    }));

    if (!response.ok) {
      return res.status(500).json({ error: `Error de la API de Claude: ${JSON.stringify(data)}` });
    }

    const toolBlock = (data.content || []).find((b) => b.type === 'tool_use');
    if (!toolBlock || !toolBlock.input || !Array.isArray(toolBlock.input.dias)) {
      return res.status(502).json({ error: 'Claude no devolvió el itinerario en el formato esperado. Prueba a generarlo otra vez.' });
    }

    return res.status(200).json({ itinerary: toolBlock.input });
  } catch (err) {
    const isTimeout = err.name === 'AbortError';
    return res.status(isTimeout ? 504 : 500).json({
      error: isTimeout ? 'Claude ha tardado demasiado en responder.' : String(err.message || err),
    });
  }
}
